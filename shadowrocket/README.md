...
```text
GY Startup AdBlock v3 - Fast No-Fill
```
...
- Xiaocan startup delay is reduced with retry-friendly 200 no-fill responses for Pangolin/CSJ, GDT, and Kuaishou ad fetch endpoints. Stats, settings, config, and logging paths are left alone or forced DIRECT to avoid retry storms. v3 also fast-acks observed GDT notice/event tail requests.
