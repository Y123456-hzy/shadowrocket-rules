...
function referencePatternHasEvidence(pattern) {
  ...
  if (pattern.id === "behavior-regression-fixtures") {
    return fixtureCases.length >= 10 && ["startup-ad-clean.js", "coolapk-clean.js", "ad-sdk-no-fill.js"].every((file) => fixtureScripts.has(file));
  }
  ...
  if (pattern.id === "remote-reference-source-check") {
    return /referenceSources\(\)/.test(releaseCheckText) &&
      /reference source is reachable/.test(releaseCheckText) &&
      /referenceRequiredSnippets\(source\)/.test(releaseCheckText) &&
      /referenceCheckUrl\(source\)/.test(releaseCheckText);
  }
...
addCheck(checks, "public reference pattern coverage", 10, referenceSources.length >= 4 && referencePatterns.length >= 9 && uniqueDuplicates(referencePatterns.map((pattern) => pattern.id)).length === 0 && referenceSources.every((source) => source.id && /^https:\/\/github\.com\//.test(source.url || "") && /^https:\/\/(?:raw\.githubusercontent\.com|github\.com)\//.test(source.check_url || "") && Array.isArray(source.must_contain) && source.must_contain.length >= 2) && referencePatterns.every(referencePatternHasEvidence), "public source manifest, required snippets, and local evidence");
